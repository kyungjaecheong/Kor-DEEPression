...
from modules_for_app import Encoding_for_model
...
# 모델 불러오기
model_depr = load_model('./tuning-models/CNN_depr.h5')
model_mdd = load_model('./tuning-models/MLP_mdd.h5')
    ...
    if request.method == 'POST':
        ...
        if pred_mode == 'depr':
            try:                
                ...
                array_encoded = np.array(values_encoded).reshape(1,-1)
                
                # 우울증 예측 (정상vs우울증)
                global percent_depr
                pred_prob_depr = model_depr.predict(array_encoded, batch_size=1, steps=1, verbose=0)
                # 확률값을 소수점 둘째자리까지 퍼센트로 변환 : 재사용을 위해 global 변수에 저장
                percent_depr = int(round(pred_prob_depr[0][0], 4)*(10**4))/100
                
                # 예측 클래스
                pred_class_depr = np.where(pred_prob_depr<0.5,0,1)
                pred_depr = pred_class_depr[0][0]
        ...
        if pred_mode == 'mdd':
            try:                
                ...
                pred_prob_mdd = model_mdd.predict(array_encoded, batch_size=1, steps=1, verbose=0)
                # 확률값을 소수점 둘째자리까지 퍼센트로 변환
                percent_mdd = int(round(pred_prob_mdd[0][0], 4)*(10**4))/100
                
                # 예측한 클래스를 문자열로 이용하기 위해 변환함
                pred_class_mdd = np.where(pred_prob_mdd<0.5,0,1)
                pred_mdd = pred_class_mdd[0][0]
                mdd_class = str()
                if pred_mdd == 0:
                    mdd_class = "경도우울증"
                elif pred_mdd == 1:
